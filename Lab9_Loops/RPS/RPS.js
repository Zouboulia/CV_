//create a function that is called when user presses button
function startGame(){
    for (attempts = 3; attempts > 0; attempts--){ //for loop that runs 3 times (= number of attempts)
        alert(attempts + " attempts left"); 
        playGame();
        if(result == "You Won!"){ //loop stops if user wins
            return;
        }
      }
}
